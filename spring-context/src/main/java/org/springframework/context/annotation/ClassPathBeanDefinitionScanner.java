...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	 ...
	 */
	/**
	 * 这个方法的作用就是上次应用中的扫描包的步骤
	 */
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		Assert.notEmpty(basePackages, "At least one base package must be specified");
		Set<BeanDefinitionHolder> beanDefinitions = new LinkedHashSet<>();
		for (String basePackage : basePackages) {
			/**扫描basePackage路径下的Java文件  并将它转换为BeanDefinition类型   findCandidateComponents() step into*/
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AbstractBeanDefinition) {
					/**
					 * 如果这个类是AbstractBeanDefinition子类
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					/**
					 * 检查并且处理常用的注解
					 * 这里的处理主要是把常用注解的值设置到AnnotationBeanDefinition当中
					 * 这里的前提是当前类必须是AnnotationBeanDefinition类型的，说白了就是加了注解的类
					 */
				...
				if (checkCandidate(beanName, candidate)) {
					...
					/**把当前 扫描包得到的BeanDefinition 注册到BeanDefinitionFactory中 */
